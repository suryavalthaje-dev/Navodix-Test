...
async function createResumeDownloadSignedUrl(profileId: string) {
  if (!profileId) {
    throw new Error('Profile ID is required.');
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'resume-download') {
      const profileIdForDownload = clean(fd.get('profile_id'));
      return json(await createResumeDownloadSignedUrl(profileIdForDownload));
    }
