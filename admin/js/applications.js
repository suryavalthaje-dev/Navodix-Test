(function () {
  ...
  async function saveInterviewRound(event) {
    ...
    try {
      ...
      if (editingInterviewId) {
        ...
        setInterviewFormMessage(`Round ${nextRound} saved successfully.`, 'success');
      }

      toggleInterviewForm(false);
      await loadInterviewRounds(selectedApplication.id);
    } catch (error) {
  ...
  function renderApplicationActivity(rows) {
    ...
    container.innerHTML = rows.map(item => {
      ...
      const actor = item.created_by_name || item.created_by_email || 'Unknown user';
  ...
  async function loadApplicationActivity(applicationId) {
    ...
    const { data, error } = await supabase
      .from('application_activity_details')
      .select('id, application_id, activity_type, from_status, to_status, note, created_by, created_at, created_by_name, created_by_email')
      .eq('application_id', applicationId)
    ...
    renderApplicationActivity(data || []);
  }
