  ...
  function renderApplicationActivity(rows) {
    ...
    container.innerHTML = `
      <div class="application-activity-table-wrap">
        <table class="application-activity-table">
          <thead>
            <tr>
              <th>By</th>
              <th>Date &amp; Time</th>
          ...
          <tbody>
            ${rows.map(item => {
              ...
              let details = item.note || '—';
              if (item.activity_type === 'status_change') {
                details = `Status changed from '${statusLabel(item.from_status || '—')}' to '${statusLabel(item.to_status || '—')}'${item.note ? ` — ${item.note}` : ''}`;
              } else if (item.activity_type === 'interview_created' || item.activity_type === 'interview_updated' || item.activity_type === 'interview_deleted') {
                const roundMatch = String(item.note || '').match(/Round\s+(\d+)/i);
                const roundText = roundMatch ? `Round ${roundMatch[1]}` : 'Interview round';
                details = `${roundText} ${item.activity_type === 'interview_created' ? 'added' : item.activity_type === 'interview_updated' ? 'updated' : 'deleted'}.`;
              }
              return `
                <tr>
                  ...
                  <td class="application-activity-details-cell">${escapeHtml(details).replace(/\n/g, '<br>')}</td>
                </tr>`;
            }).join('')}
          </tbody>
        </table>
  ...
  async function saveApplicationStatus() {
    ...
    try {
      ...
      $('applicationJobDetails').innerHTML = [
        ...
        detailItem('Job Title', updatedJob.title),
        detailItem('Job Code', updatedJob.job_code),
