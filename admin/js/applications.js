(function () {
  ...
  function renderApplications() {
    ...
    $('applicationsShowing').textContent = `Showing ${showingStart}–${showingEnd} of ${rows.length}`;
  ...
  function bindEvents() {
    ...
    $('applicationsFirst').addEventListener('click', function () {
      if (currentPage > 1) {
        currentPage = 1;
        renderApplications();
      }
    ...
    $('applicationsLast').addEventListener('click', function () {
      const totalPages = Math.max(1, Math.ceil(filteredApplications().length / PAGE_SIZE));
      if (currentPage < totalPages) {
