(function(){
  ...
  async function downloadProfileResume(profile){
    try{
      ...
      fd.append('action','resume-download');
      fd.append('profile_id',profile.id);
      const response=await fetch(window.NAVODIX_SUPABASE_URL+'/functions/v1/profile-management',{method:'POST',headers:{Authorization:'Bearer '+session.access_token},body:fd});
      const payload=await response.json().catch(()=>({}));
      if(!response.ok) throw new Error(payload.error||payload.message||'Could not prepare the resume download.');
      const url=payload.signed_url||payload.signedUrl;
      if(!url) throw new Error('The Profile Management service did not return a secure download URL.');
  ...
  async function deleteProfile(profileId, profileName){
    ...
    if(!confirmed)return;
    const confirmedAgain=window.confirm('Please confirm: permanently delete this profile?');
  ...
  function resetProfileNoteForm(){ profileNotesEditingId=''; $('profileNoteId').value=''; $('profileNoteType').value='Update'; $('profileNoteText').value=''; $('saveProfileNoteButton').innerHTML='<i class="fa-solid fa-plus"></i> Add Note'; profileNoteFormMessage(''); }
  async function loadRequirementNotes(){
    if(!profileNotesProfileId||!profileNotesRequirementId)return;
    $('profileNotesMessage').textContent='Loading…'; $('profileNotesMessage').className='admin-message'; $('profileNotesTableBody').innerHTML=''; $('profileNotesEmpty').classList.add('hidden'); $('profileNotesTableWrap').classList.remove('hidden');
    const {data,error}=await supabase.from('profile_notes').select('id,note_type,note,created_at,created_by').eq('profile_id',profileNotesProfileId).eq('requirement_id',profileNotesRequirementId).order('created_at',{ascending:false});
    if(error)throw error; const rows=data||[];
    if(!rows.length){ $('profileNotesTableWrap').classList.add('hidden'); $('profileNotesEmpty').classList.remove('hidden'); }
  ...
  async function saveProfileRequirementNote(e){
    e.preventDefault(); if(!profileNotesProfileId||!profileNotesRequirementId){profileNoteFormMessage('Requirement context is not available.','error');return;}
    const note=$('profileNoteText').value.trim(),noteType=$('profileNoteType').value;if(!note){profileNoteFormMessage('Please enter a note.','error');return;}
    const btn=$('saveProfileNoteButton');btn.disabled=true;profileNoteFormMessage(profileNotesEditingId?'Saving changes…':'Saving note…');
    try{const userId=(await supabase.auth.getUser()).data.user?.id||null;if(profileNotesEditingId){const {error}=await supabase.from('profile_notes').update({note_type:noteType,note}).eq('id',profileNotesEditingId).eq('profile_id',profileNotesProfileId).eq('requirement_id',profileNotesRequirementId);if(error)throw error;message('Requirement-specific note updated successfully.','success');}else{const {error}=await supabase.from('profile_notes').insert({profile_id:profileNotesProfileId,requirement_id:profileNotesRequirementId,note_type:noteType,note,created_by:userId});if(error)throw error;message('Requirement-specific note added successfully.','success');}resetProfileNoteForm();await loadRequirementNotes();await openProfileDetails(profileNotesProfileId);}
    catch(err){profileNoteFormMessage(err.message||String(err),'error');}finally{btn.disabled=false;}
  }
  async function openProfileDetails(id){
    profileAssociationCurrentProfileId=id;
    ...
    document.querySelectorAll('[data-profile-association-edit]').forEach(btn=>{
      btn.addEventListener('click',async function(event){
        ...
        try{ await openProfileAssociationModal(profileAssociationCurrentProfileId,this.dataset.profileAssociationEdit); }
        catch(err){ profileDetailsMessage(err.message||String(err),'error'); }
      });
    });
    const deleteBtn=$('deleteProfileButton'); if(deleteBtn)deleteBtn.addEventListener('click',()=>deleteProfile(p.id,p.full_name||p.profile_number||'this profile'));
  }
  function associationMessage(text,type=''){
    const el=$('profileAssociationMessage'); if(!el)return;
    el.textContent=text||''; el.className='admin-message'+(type?' '+type:'');
  }
  function closeProfileAssociationModal(){
    $('profileAssociationModal').classList.add('hidden');
    $('profileAssociationModal').setAttribute('aria-hidden','true');
    document.body.classList.remove('modal-open');
    profileAssociationProfileId=''; profileAssociationEditId='';
    associationMessage('');
  }
  async function openProfileAssociationModal(profileId, associationId=''){
    profileAssociationProfileId=profileId; profileAssociationEditId=associationId||'';
    const {data:p,error:pError}=await supabase.from('profiles').select('id,profile_number,full_name').eq('id',profileId).single();
    if(pError)throw pError;
    const {data:assocRows,error:aError}=await supabase.from('profile_requirement_associations')
      .select('id,requirement_id,status,notes').eq('profile_id',profileId);
    ...
    $('profileAssociationProfileName').value=`${p.full_name||''} (${p.profile_number||''})`;
    ...
    $('profileAssociationModalTitle').textContent=associationId?'Edit Requirement Association':'Add Requirement Association';
    $('saveProfileAssociationButton').innerHTML=associationId
      ? '<i class="fa-solid fa-floppy-disk"></i> Save Changes'
      : '<i class="fa-solid fa-link"></i> Associate';
    associationMessage('');
    $('profileAssociationModal').classList.remove('hidden');
    $('profileAssociationModal').setAttribute('aria-hidden','false');
    document.body.classList.add('modal-open');
  }
  async function saveProfileAssociation(){
    const reqId=$('profileAssociationRequirement').value;
    const status=$('profileAssociationStatus').value;
    const notes=$('profileAssociationNotes').value.trim();
    if(!profileAssociationProfileId){ associationMessage('Profile is not available.','error'); return; }
    if(!reqId){ associationMessage('Please select a requirement.','error'); return; }
    const btn=$('saveProfileAssociationButton'); btn.disabled=true;
    associationMessage(profileAssociationEditId?'Saving changes…':'Associating profile…');
    try{
      if(profileAssociationEditId){
        const {data:current,error:currentError}=await supabase.from('profile_requirement_associations')
        ...
        const {data:created,error}=await supabase.from('profile_requirement_associations')
          .upsert({profile_id:profileAssociationProfileId,requirement_id:reqId,status,notes:notes||null,association_type:'manual'},
            {onConflict:'profile_id,requirement_id'}).select('id,status').single();
        if(error)throw error;
        await recordInitialAssociationStatus(created.id,status,notes);
        message('Profile associated successfully with the requirement.','success');
      }
      const id=profileAssociationProfileId;
      closeProfileAssociationModal();
      await openProfileDetails(id);
      await loadProfiles();
    }catch(err){
      associationMessage(err.message||String(err),'error');
    }finally{
      btn.disabled=false;
    }
  }
  ...
  async function recordAssociationStatusHistory(associationId, previousStatus, newStatus, notes='') {
    if(!associationId || previousStatus === newStatus) return;
    const userId=(await supabase.auth.getUser()).data.user?.id||null;
    const {error}=await supabase.from('profile_requirement_status_history').insert({
  ...
  async function recordInitialAssociationStatus(associationId, status, notes='') {
  ...
  async function openAssociationStatusHistory(associationId, requirementLabel='Requirement') {
    ...
    try {
      ...
      $('associationHistoryMessage').className='admin-message error';
    }
  }
  ...
  $('profileRequirement').addEventListener('change',()=>{ $('profileAssociationRequirementId').value=$('profileRequirement').value||''; }); $('profileSource').addEventListener('change',toggleReferrerFields); $('resumeFile').addEventListener('change',()=>{selectedExistingResume=Boolean($('profileId').value); if(selectedExistingResume&&$('resumeFile').files[0])$('resumeReplaceNotice').classList.remove('hidden');}); $('keepResumeButton').addEventListener('click',()=>{keepExistingResume=true;$('resumeFile').value='';$('resumeReplaceNotice').classList.add('hidden');formMessage('Existing resume will be kept.','');}); $('replaceResumeButton').addEventListener('click',()=>{keepExistingResume=false;$('resumeReplaceNotice').classList.add('hidden');formMessage('New resume will replace the existing resume when saved.','');}); $('addProfileButton').addEventListener('click',openAdd); $('closeProfileModal').addEventListener('click',closeModal); $('cancelProfileButton').addEventListener('click',closeModal); $('profileForm').addEventListener('submit',saveProfile); $('profileSearch').addEventListener('input',()=>{currentPage=1;render();}); $('profileStatusFilter').addEventListener('change',()=>{currentPage=1;render();}); $('profileSourceFilter').addEventListener('change',()=>{currentPage=1;render();}); $('refreshProfilesButton').addEventListener('click',()=>{currentPage=1;loadProfiles();}); $('profilesFirst').addEventListener('click',()=>goToProfilesPage(1)); $('profilesPrev').addEventListener('click',()=>goToProfilesPage(currentPage-1)); $('profilesNext').addEventListener('click',()=>goToProfilesPage(currentPage+1)); $('profilesLast').addEventListener('click',()=>goToProfilesPage(Math.max(1,Math.ceil(filtered().length/PAGE_SIZE)))); $('associateExistingButton').addEventListener('click',openExistingProfileModal); $('closeExistingProfileModal').addEventListener('click',closeExistingProfileModal); $('closeAssociateRequirementModal').addEventListener('click',closeAssociateRequirementModal); $('cancelAssociateRequirementButton').addEventListener('click',closeAssociateRequirementModal); $('saveAssociateRequirementButton').addEventListener('click',saveAssociateRequirement); $('existingProfileSearch').addEventListener('input',renderExistingProfiles); $('existingProfileTableBody').addEventListener('click',async e=>{const b=e.target.closest('[data-existing-associate]');if(!b)return; b.disabled=true; try{await associateProfile(b.dataset.existingAssociate);}catch(err){$('existingProfileMessage').textContent=err.message||String(err);}finally{b.disabled=false;}}); $('profilesTableBody').addEventListener('click',async e=>{const b=e.target.closest('[data-action]');if(!b)return;try{if(b.dataset.action==='view')await openProfileDetails(b.dataset.id);else if(b.dataset.action==='edit')await editProfile(b.dataset.id);else if(b.dataset.action==='associate')await associateProfile(b.dataset.id);else if(b.dataset.action==='interviews')await openInterviews(b.dataset.id);}catch(err){message(err.message||String(err),'error');}}); $('closeProfileDetailsModal').addEventListener('click',closeProfileDetails); $('closeProfileDetailsButton').addEventListener('click',closeProfileDetails); $('closeAssociationHistoryModal').addEventListener('click',closeAssociationStatusHistory); $('closeAssociationHistoryButton').addEventListener('click',closeAssociationStatusHistory); $('profileDetailsBody').addEventListener('click',async e=>{
    ...
    if(e.target.closest('#closeProfileResumeButton')){
      const viewer=$('profileResumeViewer'); const frame=$('profileResumeFrame'); const loading=$('profileResumeLoading');
      if(frame)frame.src=''; if(loading){loading.classList.remove('hidden'); loading.textContent='Loading resume…';}
    ...
    if(edit){
      e.preventDefault();
      e.stopPropagation();
      try{ await openProfileAssociationModal(profileAssociationCurrentProfileId,edit.dataset.profileAssociationEdit); }
      catch(err){ profileDetailsMessage(err.message||String(err),'error'); }
      return;
    }
    if(e.target.closest('#addProfileAssociationButton')){
  ...
  $('profileNotesTableBody').addEventListener('click',async e=>{
    ...
    if(edit){try{const {data,error}=await supabase.from('profile_notes').select('id,note_type,note').eq('id',edit.dataset.profileNoteEdit).eq('profile_id',profileNotesProfileId).eq('requirement_id',profileNotesRequirementId).single();if(error)throw error;profileNotesEditingId=data.id;$('profileNoteId').value=data.id;$('profileNoteType').value=data.note_type||'Update';$('profileNoteText').value=data.note||'';$('saveProfileNoteButton').innerHTML='<i class="fa-solid fa-floppy-disk"></i> Save Changes';profileNoteFormMessage('Editing note…');$('profileNoteText').focus();}catch(err){$('profileNotesMessage').textContent=err.message||String(err);$('profileNotesMessage').className='admin-message error';}return;}
    const del=e.target.closest('[data-profile-note-delete]');
    if(del){if(!confirm('Delete this requirement-specific note?'))return;del.disabled=true;try{const {error}=await supabase.from('profile_notes').delete().eq('id',del.dataset.profileNoteDelete).eq('profile_id',profileNotesProfileId).eq('requirement_id',profileNotesRequirementId);if(error)throw error;message('Requirement-specific note deleted successfully.','success');resetProfileNoteForm();await loadRequirementNotes();await openProfileDetails(profileNotesProfileId);}catch(err){$('profileNotesMessage').textContent=err.message||String(err);$('profileNotesMessage').className='admin-message error';}finally{del.disabled=false;}}
  });
