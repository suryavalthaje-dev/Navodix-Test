(function () {
  ...
  function render() {
    ...
    $('locationsTableBody').innerHTML = rows.map(location => `
      <tr>
    ...
    $('emptyLocations').classList.toggle('hidden', rows.length !== 0);
    $('locationsTableBody').closest('.table-wrap').classList.toggle('hidden', rows.length === 0);
  }
  ...
  function bind() {
    ...
    $('locationStatusFilter').addEventListener('change', render);
    $('refreshLocationsButton').addEventListener('click', load);
