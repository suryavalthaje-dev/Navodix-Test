...
let actionItemLastViews=new Map(),actionItemHasUnseenChanges=new Map();
let currentPage=1;
const PAGE_SIZE=20;
...
function formatTimeOnlyLocal(v){if(!v)return '';const date=new Date(v);if(Number.isNaN(date.getTime()))return '';return date.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:true}).replace(/\b(am|pm)\b/i,m=>m.toUpperCase());}
function formatDateTime(v){if(!v)return '—';const date=new Date(v);if(Number.isNaN(date.getTime()))return '—';return date.toLocaleString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});}
...
async function load(){
  ...
  try{
    ...
    await loadActionItemChangeFlags();
...
async function loadActionItemChangeFlags(){
  actionItemLastViews=new Map();
  actionItemHasUnseenChanges=new Map();
  if(!currentUser)return;

  const {data:views,error:viewError}=await supabase
    .from('action_item_user_views')
    .select('action_item_id,last_viewed_at')
    .eq('user_id',currentUser.id);
  if(viewError)throw viewError;
  (views||[]).forEach(v=>{
    if(v.action_item_id&&v.last_viewed_at)actionItemLastViews.set(v.action_item_id,v.last_viewed_at);
  });

  // Read all activity records once.  A missing view record means the user has
  // never opened that Action Item, so all existing activity is unread.
  const [{data:history,error:historyError},{data:notes,error:notesError}]=await Promise.all([
    supabase.from('action_item_history').select('action_item_id,changed_by,created_at').order('created_at',{ascending:false}),
    supabase.from('action_item_notes').select('action_item_id,added_by,created_at').order('created_at',{ascending:false})
  ]);
  if(historyError)throw historyError;
  if(notesError)throw notesError;

  // An update is new only when it is new to this user. The person who
  // created or changed the activity must not see their own change highlighted
  // as New. Other users can still see the same activity as New until they
  // open the Action Item.
  const isUnseenForCurrentUser=(activityTime,authorId,actionItemId)=>{
    if(authorId&&currentUser?.id&&authorId===currentUser.id)return false;
    const viewed=actionItemLastViews.get(actionItemId);
    const viewedTime=viewed?new Date(viewed).getTime():NaN;
    return Number.isFinite(activityTime)&&(!viewed||!Number.isFinite(viewedTime)||activityTime>viewedTime);
  };

  (history||[]).forEach(row=>{
    const activityTime=new Date(row.created_at).getTime();
    if(isUnseenForCurrentUser(activityTime,row.changed_by,row.action_item_id)){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });
  (notes||[]).forEach(row=>{
    const activityTime=new Date(row.created_at).getTime();
    if(isUnseenForCurrentUser(activityTime,row.added_by,row.action_item_id)){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });

  // Final fallback: if an Action Item has changed but has no history/note row
  // (for example, an older item created before the history trigger existed),
  // use its updated_at timestamp. This ensures the View button still signals
  // an unread update for a user who has never opened the item.
  items.forEach(item=>{
    if(actionItemHasUnseenChanges.has(item.id))return;
    const viewed=actionItemLastViews.get(item.id);
    if(!viewed){
      const created=new Date(item.created_at||0).getTime();
      const updated=new Date(item.updated_at||0).getTime();
      // For older items without activity rows, creation by the current user
      // is not a new update for that user.
      const createdByCurrentUser=item.created_by&&currentUser?.id&&item.created_by===currentUser.id;
      if(Number.isFinite(updated)&&(!Number.isFinite(created)||updated>created)&&!createdByCurrentUser){
        actionItemHasUnseenChanges.set(item.id,true);
      }
    }else{
      const updated=new Date(item.updated_at||0).getTime();
      const viewedTime=new Date(viewed).getTime();
      if(Number.isFinite(updated)&&Number.isFinite(viewedTime)&&updated>viewedTime){
        // If there is a corresponding history row, author-aware logic above
        // already decided whether this is new. Only use updated_at as a
        // fallback when no activity row exists.
        const hasActivity=(history||[]).some(r=>r.action_item_id===item.id)||(notes||[]).some(r=>r.action_item_id===item.id);
        if(!hasActivity)actionItemHasUnseenChanges.set(item.id,true);
      }
    }
  });
}
...
function dueClass(x){if(x.status==='completed')return ' action-due-completed';if(!x.due_date)return '';const t=today();return x.due_date<t?' action-overdue':x.due_date===t?' action-due-today':'';}
function render(){const rows=filtered();const totalPages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE));if(currentPage>totalPages)currentPage=totalPages;const start=(currentPage-1)*PAGE_SIZE;const pageRows=rows.slice(start,start+PAGE_SIZE);$('actionItemsTableBody').innerHTML=pageRows.map(x=>{const unseen=actionItemHasUnseenChanges.get(x.id)===true;return `<tr class="${dueClass(x)}"><td><span class="action-title-text">${esc(x.title)}</span></td><td>${esc(x.project?.name||'—')}</td><td class="action-assignees-cell">${assigneeNamesHtml(x)}</td><td><span class="action-priority-text priority-${x.priority}">${labels[x.priority]}</span></td><td class="${dueClass(x).includes('action-due-completed')?'action-due-completed-cell':dueClass(x).includes('action-overdue')?'action-due-past':dueClass(x).includes('action-due-today')?'action-due-today-cell':''}">${formatDate(x.due_date)}</td><td><span class="action-status-text status-${x.status}">${labels[x.status]||x.status}</span></td><td class="action-updated-cell">${formatDateOnlyLocal(x.updated_at)}<span class="action-updated-time">${formatTimeOnlyLocal(x.updated_at)}</span></td><td class="action-buttons-cell"><button class="btn btn-outline btn-small application-view-button action-view-button${unseen?' action-view-button-new':''}" type="button" data-action="view" data-id="${x.id}"${unseen?' title="Updated since you last viewed this Action Item"':''}><i class="fa-solid fa-eye" aria-hidden="true"></i> View details</button></td></tr>`;}).join('');const hasRows=pageRows.length>0;$('emptyActionItems').classList.toggle('hidden',hasRows);$('actionItemsTableBody').closest('.table-wrap').classList.toggle('hidden',!hasRows);updatePagination(rows.length,totalPages,start,pageRows.length);renderSummary();}

function updatePagination(total,totalPages,start,count){
  const wrap=$('actionItemsPagination');
  if(!wrap)return;
  const hidden=total===0;
  wrap.classList.toggle('hidden',hidden);
  if(hidden)return;
  $('actionItemsPageInfo').textContent=`Showing ${start+1}-${start+count} of ${total}`;
  $('actionItemsPageNumber').textContent=`Page ${currentPage} of ${totalPages}`;
  $('actionItemsFirst').disabled=currentPage<=1;
  $('actionItemsPrev').disabled=currentPage<=1;
  $('actionItemsNext').disabled=currentPage>=totalPages;
  $('actionItemsLast').disabled=currentPage>=totalPages;
}
function goToActionItemsPage(page){
  const totalPages=Math.max(1,Math.ceil(filtered().length/PAGE_SIZE));
  currentPage=Math.min(Math.max(1,page),totalPages);
  render();
}
function goToActionItemsFirst(){
  goToActionItemsPage(1);
  window.scrollTo({top:0,behavior:'smooth'});
}
function goToActionItemsLast(){
  const totalPages=Math.max(1,Math.ceil(filtered().length/PAGE_SIZE));
  goToActionItemsPage(totalPages);
...
async function save(e){
  ...
  try{
    ...
    if(id){
      ...
      fmsg('Action item updated successfully.','success');
    }else{
      closeForm(true);
...
async function adminNames(ids){
  ...
  const result={};
  // First use the assignee directory already loaded for this page. This also
  // handles history rows written with action_item_assignees.id.
  (assignees||[]).forEach(x=>{
    if(x?.id){
      const name=x.name||x.email||null;
      if(name)result[String(x.id)]=name;
    }
  });
  if(unique.length){
    try{
      // careers_admins is intentionally protected from broad direct SELECT access.
      // Use the SECURITY DEFINER directory RPC so every Admin/HR user can resolve
      // the real creator/updater name without exposing the table directly.
      const [{data:admins,error:adminError},{data:assigneeRows,error:assigneeError}]=await Promise.all([
        supabase.rpc('get_action_item_user_directory'),
        supabase.from('action_item_assignees').select('id,name,email').in('id',unique)
      ]);
      if(adminError)console.warn('Could not resolve Action Item admin names:',adminError);
      if(assigneeError)console.warn('Could not resolve Action Item assignee names:',assigneeError);
      (assigneeRows||[]).forEach(x=>{
        if(x?.id){
          const name=x.name||x.email||null;
          if(name)result[String(x.id)]=name;
        }
      ...
      (admins||[]).forEach(x=>{
        const name=x?.display_name||x?.name||x?.full_name||x?.email||null;
        const keys=[x?.user_id,x?.id,x?.auth_user_id,x?.user_uuid].filter(Boolean);
        keys.forEach(k=>{if(name)result[String(k)]=name;});
      });
    }catch(e){
      console.warn('Could not resolve Action Item user names:',e);
    }
  }
  if(currentUser?.id&&!result[currentUser.id]){
    result[currentUser.id]=currentUser.user_metadata?.display_name||currentUser.user_metadata?.full_name||currentUser.email||null;
  }
  return result;
}
function historyValueLabel(activityType,value,assigneeMap){
  if(value===null||value===undefined||value==='')return '—';
  const type=String(activityType||'').toLowerCase();
  const key=String(value);
  if(type.includes('assigned')||type.includes('assignee')){
    const direct=assigneeMap?.[key];
    const loaded=(assignees||[]).find(a=>String(a.id)===key);
    return direct||loaded?.name||loaded?.email||'Unknown user';
  }
  if(type.includes('status'))return labels[value]||value;
  return value;
}
function activityIsNewForCurrentUser(createdAt,authorId,lastViewedAt){
  if(authorId&&currentUser?.id&&authorId===currentUser.id)return false;
  const activityTime=new Date(createdAt).getTime();
  const viewedTime=lastViewedAt?new Date(lastViewedAt).getTime():NaN;
  return Number.isFinite(activityTime)&&(!lastViewedAt||!Number.isFinite(viewedTime)||activityTime>viewedTime);
}
async function loadDetailHistory(lastViewedAt=null){
  if(!selected)return;
  ...
  try{
    ...
    if(notesError){
      ...
      $('actionNotesList').innerHTML=noteRows.length
        ? `<div class="application-activity-table-wrap"><table class="application-activity-table action-history-table"><thead><tr><th>Added By</th><th>Date / Time</th><th>Note / Update</th></tr></thead><tbody>${noteRows.map(x=>{const isNew=activityIsNewForCurrentUser(x.created_at,x.added_by,lastViewedAt);return `<tr data-note-id="${esc(x.id)}" class="${isNew?'action-activity-new':''}"><td><strong>${esc(names[x.added_by]||'Not recorded')}</strong></td><td>${formatDateTime(x.created_at)}</td><td class="application-activity-details-cell">${esc(x.note)}${isNew?' <span class="action-new-badge">New</span>':''}</td></tr>`;}).join('')}</tbody></table></div>`
    ...
    if(historyError){
      ...
      if(!rows.length&&selected.created_at){
        rows=[{
          ...
          old_value:null,
          new_value:selected.title||labels[selected.status]||selected.status||'Pending',
          changed_by:selected.created_by||null,
          created_at:selected.created_at,
          _fallback:true
        }];
      }
      // If the creation history row exists but its changed_by is null,
      // action_items.created_by is the authoritative creator.
      rows=rows.map(x=>x.activity_type==='created'&&!x.changed_by?{...x,changed_by:selected.created_by||null}:x);
      const names=await adminNames(rows.map(x=>x.changed_by));
      const assigneeIds=[];
      rows.forEach(x=>{
        const type=String(x.activity_type||'').toLowerCase();
        if(type.includes('assigned')||type.includes('assignee')){
          if(x.old_value)assigneeIds.push(x.old_value);
          if(x.new_value)assigneeIds.push(x.new_value);
        }
      });
      const assigneeNames=await adminNames(assigneeIds);
      // Ensure both historical and current assignee IDs resolve to readable names.
      (assignees||[]).forEach(a=>{
        if(a?.id&&a?.name&&!assigneeNames[String(a.id)])assigneeNames[String(a.id)]=a.name;
      });

      $('actionHistoryList').innerHTML=rows.length
        ? `<div class="application-activity-table-wrap"><table class="application-activity-table action-history-table"><thead><tr><th>Activity</th><th>Date / Time</th><th>Details</th><th>By</th></tr></thead><tbody>${rows.map(x=>{const isNew=activityIsNewForCurrentUser(x.created_at,x.changed_by,lastViewedAt);const oldLabel=historyValueLabel(x.activity_type,x.old_value,assigneeNames);const newLabel=historyValueLabel(x.activity_type,x.new_value,assigneeNames);return `<tr class="${isNew?'action-activity-new':''}"><td><strong>${esc(x.activity_type==='created'?'Created':x.description||x.activity_type)}</strong></td><td>${formatDateTime(x.created_at)}</td><td class="application-activity-details-cell">${x.old_value?`${esc(oldLabel)} <i class="fa-solid fa-arrow-right"></i> `:''}${esc(newLabel)}${isNew?' <span class="action-new-badge">New</span>':''}</td><td>${esc(names[x.changed_by]||'Not recorded')}</td></tr>`;}).join('')}</tbody></table></div>`
        : '<div class="application-activity-empty">No history is available.</div>';
    }
    if(!notesError&&!historyError&&currentUser){
      await markActionItemViewed(actionItemId);
    }
  }catch(e){
...
async function markActionItemViewed(actionItemId){
  if(!currentUser||!actionItemId)return;
  const viewedAt=new Date().toISOString();
  const {error}=await supabase.from('action_item_user_views').upsert({user_id:currentUser.id,action_item_id:actionItemId,last_viewed_at:viewedAt},{onConflict:'user_id,action_item_id'});
  if(error)throw error;
  actionItemLastViews.set(actionItemId,viewedAt);
  actionItemHasUnseenChanges.delete(actionItemId);
  render();
}
async function addNote(e){
  ...
  try{
    ...
    await loadDetailHistory(actionItemLastViews.get(actionItemId)||null);
...
function applyQuickFilter(type){currentPage=1;quickFilter=type;$('actionSearch').value='';$('actionStatusFilter').value='';$('actionPriorityFilter').value='';$('actionAssigneeFilter').value='';$('actionProjectFilter').value='';$('actionDueDateFilter').value='';updateActionFilterHighlights();updateQuickFilterButtons();render();}
function clearFilters(){currentPage=1;$('actionSearch').value='';$('actionStatusFilter').value='';$('actionPriorityFilter').value='';$('actionAssigneeFilter').value='';$('actionProjectFilter').value='';$('actionDueDateFilter').value='';quickFilter='';updateActionFilterHighlights();updateQuickFilterButtons();render();}
function bind(){$('addActionItemButton').addEventListener('click',()=>openForm());$('actionPrimaryAssignee').addEventListener('change',()=>refreshSecondaryAssigneeOptions());$('allIncompleteItemsButton').addEventListener('click',()=>applyQuickFilter('all-incomplete'));$('myIncompleteItemsButton').addEventListener('click',()=>applyQuickFilter('my-incomplete'));$('closeActionModal').addEventListener('click',closeForm);$('cancelActionButton').addEventListener('click',closeForm);$('actionItemForm').addEventListener('submit',save);$('actionSearch').addEventListener('input',()=>{currentPage=1;quickFilter='';updateQuickFilterButtons();render();});['actionStatusFilter','actionPriorityFilter','actionAssigneeFilter','actionProjectFilter','actionDueDateFilter'].forEach(id=>$(id).addEventListener('change',()=>{currentPage=1;quickFilter='';updateActionFilterHighlights();updateQuickFilterButtons();render();}));$('clearActionFilters').addEventListener('click',clearFilters);$('refreshActionItems').addEventListener('click',()=>{currentPage=1;load();});$('actionItemsFirst').addEventListener('click',goToActionItemsFirst);$('actionItemsPrev').addEventListener('click',()=>goToActionItemsPage(currentPage-1));$('actionItemsNext').addEventListener('click',()=>goToActionItemsPage(currentPage+1));$('actionItemsLast').addEventListener('click',goToActionItemsLast);$('actionItemsTableBody').addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b)return;if(b.dataset.action==='view')openDetail(b.dataset.id);});$('closeActionDetail').addEventListener('click',closeDetail);$('actionDetailCloseBtn').addEventListener('click',closeDetail);$('actionDetailEditBtn').addEventListener('click',()=>{if(!selected)return;openForm(selected,true);});$('actionDetailDeleteBtn').addEventListener('click',deleteSelectedActionItem);$('actionNoteForm').addEventListener('submit',addNote);document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(!$('actionItemModal').classList.contains('hidden'))closeForm();else if(!$('actionDetailModal').classList.contains('hidden'))closeDetail();});}
