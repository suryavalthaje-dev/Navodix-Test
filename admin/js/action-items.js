...
async function loadActionItemChangeFlags(){
  ...
  if(!actionItemLastViews.size)return;
  ...
  (history||[]).forEach(row=>{
    ...
    if(viewed&&new Date(row.created_at).getTime()>new Date(viewed).getTime()&&row.changed_by!==currentUser.id){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });
  (notes||[]).forEach(row=>{
    const viewed=actionItemLastViews.get(row.action_item_id);
    if(viewed&&new Date(row.created_at).getTime()>new Date(viewed).getTime()&&row.added_by!==currentUser.id){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });
}
