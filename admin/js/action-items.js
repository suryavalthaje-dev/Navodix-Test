...
async function load(){
  ...
  try{
    const [userResult,aResult,pResult,iResult,mResult]=await Promise.all([
      ...
      supabase.from('action_item_assignee_map').select('action_item_id,assignee_id,created_at').order('created_at')
    ]);
    ...
    mapRows.forEach(r=>{if(!grouped.has(r.action_item_id))grouped.set(r.action_item_id,[]);const a=byId.get(r.assignee_id);if(a)grouped.get(r.action_item_id).push(a);});
    items=(iResult.data||[]).map(x=>{
      let assignedPeople=grouped.get(x.id)||[];
      if(!assignedPeople.length&&x.assigned_to){const legacy=byId.get(x.assigned_to);if(legacy)assignedPeople=[legacy];}
      return {...x,assignees:assignedPeople,assignee:assignedPeople[0]||x.assignee||null};
    });
    ...
    populateMultiSelect('actionAssignee',assignees.filter(x=>x.is_active));
...
function getSelectedAssigneeIds(){return Array.from($('actionAssignee').selectedOptions||[]).map(o=>o.value).filter(Boolean);}
function setSelectedAssigneeIds(ids){
  const set=new Set(ids||[]);
  Array.from($('actionAssignee').options).forEach(o=>{o.selected=set.has(o.value);});
}
...
function openForm(item=null,fromDetail=false){returnToDetailAfterForm=Boolean(item&&fromDetail);$('actionItemModal').classList.toggle('action-edit-overlay',returnToDetailAfterForm);$('actionItemForm').reset();$('actionItemId').value=item?.id||'';$('actionModalTitle').textContent=item?'Edit Action Item':'Add Action Item';$('actionTitle').value=item?.title||'';$('actionDescription').value=item?.description||'';$('actionProject').value=item?.project_area_id||'';setSelectedAssigneeIds((item?.assignees||[]).map(a=>a.id).filter(Boolean));$('actionPriority').value=item?.priority||'medium';$('actionDueDate').value=item?.due_date||'';$('actionStatus').value=item?.status||'pending';fmsg('');$('actionItemModal').classList.remove('hidden');$('actionItemModal').setAttribute('aria-hidden','false');document.body.classList.add('modal-open');$('actionTitle').focus();}
...
async function save(e){
  ...
  const selectedAssigneeIds=getSelectedAssigneeIds();
  ...
  try{
    ...
    const primaryAssignee=selectedAssigneeIds[0]||null;
    ...
    if(selectedAssigneeIds.length){
      const rows=selectedAssigneeIds.map(assignee_id=>({action_item_id:actionId,assignee_id}));
