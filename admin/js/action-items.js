...
async function loadDetailHistory(lastViewedAt=null){
  ...
  try{
    ...
    if(historyError){
      ...
      const creatorIds=[...historyRows.map(x=>x.changed_by), selected.created_by].filter(Boolean);
      const names=await adminNames(creatorIds);
