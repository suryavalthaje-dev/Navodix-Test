...
async function loadActionItemChangeFlags(){
  ...
  (views||[]).forEach(v=>{if(v.action_item_id&&v.last_viewed_at)actionItemLastViews.set(v.action_item_id,v.last_viewed_at);});

  // If a user has never viewed any Action Item, there is no view timestamp to
  // use as a lower bound. In that case, inspect all history/notes so existing
  // updates by other users are shown as unread on the user's first login.
  const timestamps=[...actionItemLastViews.values()].map(v=>new Date(v).getTime()).filter(Number.isFinite);
  const oldest=timestamps.length?new Date(Math.min(...timestamps)).toISOString():null;
  const historyQuery=oldest
    ? supabase.from('action_item_history').select('action_item_id,changed_by,created_at').gt('created_at',oldest)
    : supabase.from('action_item_history').select('action_item_id,changed_by,created_at');
  const notesQuery=oldest
    ? supabase.from('action_item_notes').select('action_item_id,added_by,created_at').gt('created_at',oldest)
    : supabase.from('action_item_notes').select('action_item_id,added_by,created_at');
  const [{data:history,error:historyError},{data:notes,error:notesError}]=await Promise.all([historyQuery,notesQuery]);
  if(historyError)throw historyError;
  if(notesError)throw notesError;
  (history||[]).forEach(row=>{
    const viewed=actionItemLastViews.get(row.action_item_id);
    if(row.changed_by!==currentUser.id&&(!viewed||new Date(row.created_at).getTime()>new Date(viewed).getTime())){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });
  (notes||[]).forEach(row=>{
    const viewed=actionItemLastViews.get(row.action_item_id);
    if(row.added_by!==currentUser.id&&(!viewed||new Date(row.created_at).getTime()>new Date(viewed).getTime())){
      actionItemHasUnseenChanges.set(row.action_item_id,true);
    }
  });
}
