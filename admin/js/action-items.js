...
async function adminNames(ids){
  ...
  try{
    const [{data:admins,error:adminError},{data:assigneeRows,error:assigneeError}]=await Promise.all([
      supabase.from('careers_admins').select('user_id,display_name,email').in('user_id',unique),
    ...
    (admins||[]).forEach(x=>{if(x.user_id)result[x.user_id]=x.display_name||x.email||null;});
  }catch(e){console.warn('Could not resolve Action Item user names:',e);}
