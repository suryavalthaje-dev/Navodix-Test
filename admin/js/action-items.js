...
async function load(){msg('Loading action items…');try{const [{data:userData,error:userError},{data:a,error:aError},{data:p,error:pError},{data:i,error:iError}]=await Promise.all([supabase.auth.getUser(),supabase.from('action_item_assignees').select('id,name,email,is_active').order('name'),supabase.from('projects_areas').select('id,name,is_active').order('name'),supabase.from('action_items').select('*,project:project_area_id(id,name),assignee:assigned_to(id,name,is_active)').order('updated_at',{ascending:false})]);if(userError)throw userError;if(aError)throw aError;if(pError)throw pError;if(iError)throw iError;currentUser=userData?.user||null;assignees=a||[];projects=p||[];items=i||[];await resolveCurrentUserAssignee();populateSelect('actionAssigneeFilter',assignees.filter(x=>x.is_active),'All Assignees');populateSelect('actionProjectFilter',projects.filter(x=>x.is_active),'All Projects / Areas');populateSelect('actionAssignee',assignees.filter(x=>x.is_active),'Select Assignee');updateActionFilterHighlights();updateQuickFilterButtons();populateSelect('actionProject',projects.filter(x=>x.is_active),'Select Project / Area');render();msg('');}catch(e){console.error(e);msg(`Could not load action items: ${e.message||e}`,'error');}}
...
function filtered(){const q=$('actionSearch').value.trim().toLowerCase(),s=$('actionStatusFilter').value,p=$('actionPriorityFilter').value,a=$('actionAssigneeFilter').value,pr=$('actionProjectFilter').value,d=$('actionDueDateFilter').value;return items.filter(x=>{const hay=`${x.title} ${x.description||''} ${x.project?.name||''} ${x.assignee?.name||''}`.toLowerCase();const incomplete=x.status==='pending'||x.status==='in_progress';const quickMatch=!quickFilter||(incomplete&&(quickFilter==='all-incomplete'||(quickFilter==='my-incomplete'&&currentUserAssigneeId&&x.assigned_to===currentUserAssigneeId)));return quickMatch&&(!q||hay.includes(q))&&(!s||x.status===s)&&(!p||x.priority===p)&&(!a||x.assigned_to===a)&&(!pr||x.project_area_id===pr)&&(!d||x.due_date===d);});}
