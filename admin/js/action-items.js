...
async function adminNames(ids){
  ...
  try{
    ...
    (assigneeRows||[]).forEach(x=>{if(x.id)result[x.id]=x.name||x.email||null;});
    (admins||[]).forEach(x=>{if(x.user_id)result[x.user_id]=x.display_name||null;});
  }catch(e){
...
function historyValueLabel(activityType,value,assigneeMap){
  ...
  if(type.includes('assigned')||type.includes('assignee'))return assigneeMap[value]||assignees.find(a=>a.id===value)?.name||value;
