(function () {
  ...
  function renderJobs() {
      ...
      <tr>
        ...
        <td>
          <div class="row-actions">
            <button class="icon-button" type="button" data-action="view" data-id="${job.id}" title="View job" aria-label="View job">
              <i class="fa-solid fa-eye"></i>
            </button>
            <button class="icon-button" type="button" data-action="edit" data-id="${job.id}" title="Edit job" aria-label="Edit job">
              <i class="fa-solid fa-pen-to-square"></i>
            </button>
            <button class="icon-button danger-icon" type="button" data-action="delete" data-id="${job.id}" title="Delete job" aria-label="Delete job">
    ...
    body.parentElement.classList.toggle('hidden', rows.length === 0);
    updateSummary();
  }
  ...
  async function loadJobs() {
    ...
    try {
      ...
      renderJobs();
      showMessage(jobs.length ? '' : 'No jobs have been created yet.', '');
    } catch (error) {
      console.error(error);
      showMessage(`Could not load jobs: ${error.message || error}`, 'error');
    }
  }
  ...
  function setViewMode(enabled) {
    ...
    $('jobModalTitle').textContent = enabled ? 'View Job' : ($('jobId').value ? 'Edit Job' : 'Add New Job');
  }
  ...
  function openModal(job = null, mode = 'edit') {
    ...
    if (job) {
      ...
      $('jobModalTitle').textContent = 'Add New Job';
  ...
  function closeModal(force = false) {
    if (!force && jobFormDirty) {
      const discard = window.confirm('You have entered information for this job. Are you sure you want to close without saving?');
  ...
  async function saveJob(event) {
    ...
    try {
      ...
      console.error(error);
      showFormMessage(`Could not save job: ${error.message || error}`, 'error');
    } finally {
      saving = false;
      button.disabled = false;
      button.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Job';
    }
  }
  ...
  async function deleteJob() {
    ...
    try {
      ...
      console.error(error);
      showMessage(`Could not delete job: ${error.message || error}`, 'error');
    } finally {
      button.disabled = false;
      button.innerHTML = '<i class="fa-solid fa-trash"></i> Delete Job';
    }
  }
